...
const head=['EMAIL','RAGIONE SOCIALE','CODICE CLIENTE','CITTA','PROVINCIA','REGIONE','AGENTE','CLASSE ABC','STATO','TIPO CLIENTE','ANNI MACCHINA','ULTIMA MACCHINA','VENDITE','ORDINI APERTI'];
const body=rows.map(({c,em})=>{const a=macAgeYears(c);return [em,c.name,c.id,c.city||'',c.province||'',regionOf(provOf(c)),c.agent||'',c.abc||'',clientStatus(c).status||'',clientType(c).label||'',a!=null?a.toFixed(1).replace('.',','):'',lastMacDesc(c),Math.round(c.sales||0),Math.round(c.orders||0)].map(csvCell).join(',')});
...
const SW_EXPECTED='maps-app-v13-0-client-type';
...
function clientType(c){if(!hasClassData())return{tipo:'',label:'',ev:[],last:0};
 const ev=macEvents(c);const acc=(c.saleLines||[]).some(l=>clsKind(l.cls)==='acc');
 const last=ev.length?ev[ev.length-1]:0;
 let tipo='';
 if(ev.length>=7)tipo='rivenditore';else if(ev.length>=2)tipo='ricorrente';else if(ev.length===1)tipo='utilizzatore';else if(acc)tipo='accessori';
 return{tipo,label:TIPO[tipo]||'',ev,last}}
...
function filtered(){const q=norm($('#search').value).toLowerCase(),ag=$('#agentFilter').value;return Object.values(project.clients).filter(c=>(!q||[c.name,c.city,c.province,c.id].join(' ').toLowerCase().includes(q))&&(!ag||c.agent===ag)&&(!geoSel.regions.size||geoSel.regions.has(regionOf(provOf(c))))&&(!geoSel.provinces.size||geoSel.provinces.has(provOf(c)))&&(!$('#onlyOrders').checked||c.orders>0)&&(!$('#onlySales').checked||c.sales>0)&&(!$('#onlyMissing').checked||c.lat==null)&&(!$('#statusFilter').value||clientStatus(c).status===$('#statusFilter').value)&&matchType(c)&&matchAge(c)&&(!hasTransactionFilter()||matchingLines(c).length>0))}
function matchType(c){const v=$('#typeFilter')?.value;return !v||clientType(c).tipo===v}
