...
function isFin(c){try{if((c&&c.bizType||'')==='finanziaria')return true;return /\b(LEASING|NOLEGG|RENT|RENTAL|FACTOR|LOCAZION|LOCAT)\b/i.test((c&&c.name)||'')}catch(_){return false}}
function isTop(c){try{if(isFin(c))return false;const w=WIN.get(c.id);if(!w)return false;const cur=w.ya||0,prev=w.yb||0;
 if(cur<TOP_VOL_MIN)return false;const cresc=cur-prev;if(cresc<TOP_GROW_MIN)return false;
 if(prev<=0)return cur>=TOP_GROW_MIN;return cresc/prev>=TOP_GROW_PCT}catch(_){return false}}
...
$('#excelInput').onchange=e=>importFiles([...e.target.files]);$('#projectInput').onchange=async e=>{try{const p=JSON.parse(await e.target.files[0].text());if(!p.clients)throw 0;
 ...
 project=p;try{migrateAgentBase()}catch(_){}try{render()}catch(_){}try{fit()}catch(_){}try{await save()}catch(err){console.warn('save fallito, dati comunque caricati',err)}}catch(err){console.error('apertura:',err);alert('File non leggibile come progetto: '+(err&&err.message?err.message:err))}finally{e.target.value=''}};$('#exportBtn').onclick=exportProject;
