...
const body=rows.map(({c,em})=>{const a=macAgeYears(c);return [em,c.name,c.id,c.city||'',c.province||'',regionOf(provOf(c)),c.agent||'',c.abc||'',clientStatus(c).status||'',bizOf(c)?BIZ[bizOf(c)]:(guessBiz(c)?BIZ[guessBiz(c)]:''),bizOf(c)?'impostato':(guessBiz(c)?'suggerito dal nome':''),behaviorOf(c).label,a!=null?a.toFixed(1).replace('.',','):'',lastMacDesc(c),Math.round(c.sales||0),Math.round(c.orders||0)].map(csvCell).join(',')});
...
const SW_EXPECTED='maps-app-v13-3-scheda-tipo';
