...
function isFin(c){if((c.bizType||'')==='finanziaria')return true;
 return /\b(LEASING|NOLEGG|RENT|RENTAL|FACTOR|LOCAZION|LOCAT)\b/i.test(c.name||'')}
function isTop(c){if(isFin(c))return false;const w=WIN.get(c.id);if(!w)return false;const cur=w.ya||0,prev=w.yb||0;
 if(cur<TOP_VOL_MIN)return false;                          // volume minimo quest'anno
 const cresc=cur-prev;
 if(cresc<TOP_GROW_MIN)return false;                       // crescita minima in euro
 if(prev<=0)return cur>=TOP_GROW_MIN;                       // nuovo/ripartito da zero
 return cresc/prev>=TOP_GROW_PCT}                           // crescita minima in %
function topInfo(c){const w=WIN.get(c.id);if(!w)return null;const cur=w.ya||0,prev=w.yb||0;return{cur,prev,delta:cur-prev,pct:prev>0?(cur-prev)/prev*100:null}}
...
const beforeCutoff=(t)=>{const m=t.getUTCMonth(),d=t.getUTCDate();return m<mm||(m===mm&&d<=dd)};   // stesso giorno/mese o prima
...
for(const l of c.saleLines||[]){const t=parseDMY(l.date);if(!t)continue;dated=true;if(t>last)last=t;const v=num(l.amount);
 ...
 // confronto anno-su-anno a pari data (per il marker "in crescita")
